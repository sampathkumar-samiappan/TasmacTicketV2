...
import {
    ...
    Pagination,
    Drawer,Tooltip,
    message, List, Typography, Flex
} from 'antd';
import {
    PlusOutlined,
    EditOutlined,
    DeleteOutlined,
    SearchOutlined,ClockCircleTwoTone
...
const { Text } = Typography;
    ...
    const fetchOffice = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/api/resource/office?fields=[%22*%22]`, 
          { method: 'GET', headers: HEADERS }
        );
        const jsonData = await response.json();
...
>
 ...
 {view === 'asset' && (
  <>
    ...
    >
      <Select
        onChange={value => setOfficeCategory(value)}
        allowClear
        disabled={isEditMode}
      >
        {[...new Set(officeData.map(item => item.office_category))].map(cat => (
    ...
    {officeCategory && (
      ...
      >
        <Select
          ...
          disabled={isEditMode}
        >
          {regionList.map(r => <Select.Option key={r} value={r}>{r}</Select.Option>)}
        </Select>
      </Form.Item>
    )}
    ...
    {region && (
      ...
      >
        <Select
          onChange={value => setDm(value)}
          value={dm}
          allowClear
          disabled={isEditMode}
        >
          {dmList.map(d => <Select.Option key={d} value={d}>{d}</Select.Option>)}
        </Select>
      </Form.Item>
    )}
    ...
    {dm && (
      ...
      >
        <Select
          ...
          disabled={isEditMode}
        >
          {depotList.map(dep => <Select.Option key={dep} value={dep}>{dep}</Select.Option>)}
        </Select>
      </Form.Item>
    )}
    ...
    {officeCategory === "RV SHOP" && depot && (
      ...
      >
        <Select
          ...
          disabled={isEditMode}
        >
          {shopNumberList.map(sn => <Select.Option key={sn} value={sn}>{sn}</Select.Option>)}
        </Select>
      </Form.Item>
    )}
    ...
    >
      ...
      >
        {assetCategoryData.map(item => (
          <Select.Option key={item.name} value={item.assets_category}>
    ...
    <Form.Item
      ...
      rules={isEditMode ? [] : [{ required: true }]}
    >
      <Select placeholder="Select Asset Type" disabled={isEditMode}>
        {assetTypeData.map(item => (
          <Select.Option key={item.name} value={item.asset_type}>
            {item.asset_type}
          </Select.Option>
        ))}
      </Select>
    ...
    >
      <Select value={status} onChange={(value) => setStatus(value)}>
        ...
        <Select.Option value="Scrap">Move to Scrap</Select.Option>
      </Select>
    </Form.Item>
